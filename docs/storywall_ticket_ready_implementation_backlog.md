...
| **M5-T05** | E5 | Research package synthesis from bounded retrieval outputs | After candidates persist, worker runs deterministic `synthesizeResearchPackageV1` → JSON `research_synthesis_package` on `research_artifact` with sourced vs summary vs gap findings, UUID provenance, partial/stub honesty; API exposes field on research package read; **no** LLM transport | M5-T04 |
| **M5-T06** | E5 | Chronology pipeline v2 on real packages | `buildChronologyEventsFromResearchPackage` (or successor) may consume `research_synthesis_package` alongside summaries; golden tests for shape parity | M5-T05, M2-T03 |
