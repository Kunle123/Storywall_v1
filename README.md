...
### M5-T06 local E2E (research → synthesis → chronology + framing + live enrichment + editorial review audit)

Checked-in script: `apps/api/scripts/e2e-m5-t06-research-chronology.mjs` (also `pnpm --filter @storywall/api run e2e:m5-t06`).

**What it proves:** bounded retrieval (live when configured), persisted `research_synthesis_package` (`m5-t05-v1`), chronology extraction at `m5-t06-v1` with M5-T06 `context_label` / `creator_note` provenance and `[temporal]` honesty markers, plus persisted **`draft_enrichment_package`** (`m5-t08-v1`: M5-T07 scaffolding + **M5-T08** per-node `provenance` / `support_status`), companion **`draft_enrichment_provenance`** flat index, and **`honesty_summary`** (`m5-t09-v1`) on `GET …/package`. It also asserts **`ai_framing_generation`** (`m5-t10-v1`) on **`POST …/frames/generate`** (initial + post-research regenerate), including embedded **M5-T09** honesty context and `research_job_id` linkage after research. **M5-T11** adds **`POST …/research/jobs/:jobId/live-event-draft-enrichment/generate`**, persisted on **`story_brief.ai_event_draft_enrichment_package`**, echoed as **`live_event_draft_enrichment`** on **`GET …/package`** when the stored envelope’s `research_job_id` matches the requested job. **M5-T12** adds **`POST …/research/jobs/:jobId/editorial-review/generate`**, persisted on **`story_brief.ai_editorial_review_package`**, echoed as **`ai_editorial_review`** on **`GET …/package`** when scoped to the same job (live model path or **deterministic honesty/coverage-gap fallback** or empty failure envelope).

**Operational:** restart **API and worker** after deploy so the queue and HTTP layer serve the current response contract; keep **API and worker on the same `REDIS_URL` logical DB** (see isolation rule below). After schema changes (e.g. `ai_framing_generation_package`, **`ai_event_draft_enrichment_package`**, **`ai_editorial_review_package`** for M5-T12), run **`pnpm db:deploy`** (or `prisma migrate deploy`) before E2E.

**Optional live AI (M5-T10 / M5-T11 / M5-T12):** use the same **`STORYWALL_AI_*`** variables on the **API** process only (`STORYWALL_AI_RUNTIME_ENABLED=true`, `STORYWALL_AI_PROVIDER=openai_compatible`, `STORYWALL_AI_BASE_URL`, `STORYWALL_AI_API_KEY`). If unset or transport fails, the script still passes using **honest fallbacks** (deterministic framing; M5-T11 draft mapping or empty failure; M5-T12 **honesty/coverage-gap-derived** findings or empty failure).

**Prerequisites**

- **Postgres:** use an empty or dedicated database for this flow; run `pnpm db:deploy` (or `pnpm db:migrate`) so schema includes M5-T05 (`research_synthesis_package`), **M5-T10** (`story_brief.ai_framing_generation_package`), **M5-T11** (`story_brief.ai_event_draft_enrichment_package`), and **M5-T12** (`story_brief.ai_editorial_review_package`).
...
Success ends with `OK: M5-T06 research → synthesis → chronology → M5-T07/M5-T08 draft enrichment + provenance + M5-T09 honesty + M5-T10 ai_framing_generation + M5-T11 live_event_draft_enrichment + M5-T12 ai_editorial_review e2e passed.`
