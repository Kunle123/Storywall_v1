#!/usr/bin/env node
/**
 * M5-T23 — Staging: funnel through draft edit, POST …/validation/run (sync), GET …/validation/latest,
 * then PATCH event again to prove editing still persists after checks.
 *
 * Usage: pnpm verify:staging:validation-checks
 */
...
async function main() {
  const email = `staging-validation-${Date.now()}@example.test`;
  const password = "StagingValidation9!";
  ...
  let eventId = null;
  let eventEtag = null;
  const marker1 = `M5-T23-pre-${Date.now()}`;
  const marker2 = `M5-T23-post-${Date.now()}`;

  // 1 Register
  {
    const url = `${STAGING_BASE}/api/v1/auth/register`;
    const bodyIn = { email, password, displayName: "M5-T23 validation checks" };
    let status = null;
    let body = null;
    try {
  ...
  // 3 Create story
  {
    ...
    const bodyIn = {
      subject: "M5-T23 validation subject",
      story_type: "biography",
      research_brief: "Brief for validation verify.",
      desired_angle: "Validation verify angle.",
    ...
    try {
      const res = await fetch(url, {
        ...
        body: JSON.stringify({
          ...
          notes: "M5-T23 validation verify",
        }),
      });
      status = res.status;
      const rb = await readBody(res);
    ...
    try {
      const res = await fetch(url, {
        ...
        headers: {
          "Content-Type": "application/json",
          ...auth,
          "Idempotency-Key": `m5t23-select-${Date.now()}`,
        },
  ...
  // 9 POST draft/assemble
  {
    ...
    try {
      const res = await fetch(url, {
        ...
        headers: {
          "Content-Type": "application/json",
          ...auth,
          "Idempotency-Key": `m5t23-assemble-${Date.now()}`,
        },
        body: JSON.stringify({
    ...
    const ok = isHttpSuccess(status) && body?.ok === true && body?.data?.job_id;
    if (ok) assembleJobId = body.data.job_id;
  ...
  // 11 GET events + PATCH (pre-validation edit)
  {
    ...
    const ok =
      isHttpSuccess(status) && body?.ok === true && events.length > 0 && first?.id && first?.updated_at;
    if (ok) {
      eventId = first.id;
      eventEtag = first.updated_at;
    }
    record("11 — GET events (pick row for edit)", "GET", url, status, ok ? "passed on staging" : "attempted on staging but failed", body);
  }
  if (!eventId || !eventEtag) {
    record("blocked", "—", "—", null, "blocked on staging", null, "No event.");
    printTable(steps);
    process.exit(1);
  }

  {
    ...
    try {
      const res = await fetch(url, {
        ...
        body: JSON.stringify({ dek: marker1 }),
      });
      status = res.status;
      const rb = await readBody(res);
      body = rb.parsed ?? rb.raw;
    ...
    const ok = isHttpSuccess(status) && body?.ok === true && ev?.dek === marker1;
    if (ok && ev?.updated_at) eventEtag = ev.updated_at;
    record("12 — PATCH event (pre-checks edit)", "PATCH", url, status, ok ? "passed on staging" : "attempted on staging but failed", body);
  }

  let postCheckWorkflow = null;
  let postCheckOverall = null;

  // 13 POST validation/run — synchronous mutation §17.1
  {
    ...
    try {
      const res = await fetch(url, {
        ...
        headers: {
          "Content-Type": "application/json",
          ...auth,
          "Idempotency-Key": `m5t23-validation-${Date.now()}`,
        },
        body: JSON.stringify(bodyIn),
      });
      status = res.status;
    ...
    const ok =
      isHttpSuccess(status) &&
      body?.ok === true &&
      d?.validation_report_id &&
      d?.overall_result &&
      (d?.story_state === "blocked" || d?.story_state === "ready_to_publish");
    if (ok) {
      postCheckWorkflow = d.story_state;
      postCheckOverall = d.overall_result;
    }
    record(
      "13 — POST validation/run (sync checks)",
      "POST",
      url,
      status,
      ok ? "passed on staging" : "attempted on staging but failed",
      body,
      ok ? "Expect story_state blocked | ready_to_publish per engine." : undefined,
    );
  }

  // 14 GET validation/latest — same contract as Draft tab
  {
    ...
    const ok =
      isHttpSuccess(status) &&
      body?.ok === true &&
      d?.has_validation_run === true &&
      rep?.id &&
      rep?.overall_result &&
      Array.isArray(issues);
    record(
      "14 — GET validation/latest (report + issues)",
      "GET",
      url,
      status,
      ok ? "passed on staging" : "attempted on staging but failed",
      {
        story_state: d?.story_state,
        overall_result: rep?.overall_result,
        issue_count: issues.length,
        summary_note: rep?.summary_note,
      },
      ok ? `Issues returned: ${issues.length}` : undefined,
    );
  }

  // 15 GET framing — workflow matches run
  {
    const url = `${STAGING_BASE}/api/v1/creator/stories/${encodeURIComponent(storyId)}/framing`;
    let status = null;
    let body = null;
    ...
    const wf = body?.data?.story_state;
    const ok = isHttpSuccess(status) && body?.ok === true && wf === postCheckWorkflow;
    record(
      "15 — GET framing (workflow after checks)",
      "GET",
      url,
      status,
      ok ? "passed on staging" : "attempted on staging but failed",
      body,
      ok ? `Matches POST run: ${wf}` : `Expected ${postCheckWorkflow}, got ${wf}`,
    );
  }

  // 16 PATCH event after checks — editorial edits still allowed (M5-T22 states)
  {
    const url = `${STAGING_BASE}/api/v1/creator/stories/${encodeURIComponent(storyId)}/events/${encodeURIComponent(eventId)}`;
    let status = null;
    let body = null;
    try {
      const res = await fetch(url, {
        ...
        body: JSON.stringify({ dek: marker2 }),
      });
      status = res.status;
      const rb = await readBody(res);
      body = rb.parsed ?? rb.raw;
    ...
    const ev = body?.data?.event_draft;
    const ok = isHttpSuccess(status) && body?.ok === true && ev?.dek === marker2;
    if (ok && ev?.updated_at) eventEtag = ev.updated_at;
    record(
      "16 — PATCH event (post-checks edit)",
      "PATCH",
      url,
      status,
      ok ? "passed on staging" : "attempted on staging but failed",
      body,
      ok ? "Proves API still accepts event PATCH in blocked | ready_to_publish." : undefined,
    );
  }

  // 17 GET events — persistence
  {
    const url = `${STAGING_BASE}/api/v1/creator/stories/${encodeURIComponent(storyId)}/events`;
    let status = null;
    let body = null;
    ...
    const ok =
      isHttpSuccess(status) && body?.ok === true && found && String(found.dek ?? "") === marker2;
    record(
      "17 — GET events (re-read post-checks edit)",
      "GET",
      url,
      status,
      ok ? "passed on staging" : "attempted on staging but failed",
      { ...body, data: { ...body?.data, events: `[${events.length} rows; dek=${found?.dek}]` } },
      ok ? undefined : "Persistence failed after checks.",
    );
  }

  record(
    "18 — Next creator action",
    "—",
    "—",
    null,
    "passed on staging",
    {
      contract: [
        "POST /api/v1/creator/stories/:id/validation/run",
        "GET /api/v1/creator/stories/:id/validation/latest",
      ],
      after_checks: postCheckWorkflow,
      overall: postCheckOverall,
      next:
        postCheckWorkflow === "blocked"
          ? "Fix blockers listed under Readiness, edit manuscript, then Run checks again."
          : "Review warnings in the issue list if any; continue drafting; publish only when you intentionally use the publish flow (warnings may require acknowledgment).",
    },
    "Checks are not publication.",
  );
  ...
  console.log("\nM5-T23 staging verify: validation run + latest snapshot + post-check edit passed.");
}
