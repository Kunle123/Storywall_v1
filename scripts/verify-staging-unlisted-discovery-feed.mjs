#!/usr/bin/env node
/**
 * M5-T27 — Staging: first publish (public) → story appears on GET /api/v1/stories/discover;
 * republish to unlisted → slug GET still 200 but discover omits slug;
 * republish to private → slug 404 and discover still omits.
 *
 * Discovery surface: GET /api/v1/stories/discover (public live visibility only).
 *
 * Usage: pnpm verify:staging:unlisted-discovery-feed
 */
...
async function runValidationForRepublish(token, storyId, record, stepPrefix) {
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    ...
    const { status, body } = await fetchJson("POST", url, {
      ...
      idempotencyKey: `m5t27-reval-${attempt}-${Date.now()}`,
    });
    const d = body?.data;
    const ok =
...
async function patchDraftVisibility(token, storyId, visibility_target, record, stepLabel) {
  ...
  const { status: st, body: b } = await fetchJson("PATCH", url, {
    ...
    idempotencyKey: `m5t27-vis-${visibility_target}-${Date.now()}`,
...
async function main() {
  const marker = `m5-t27-discover-${Date.now()}`;
  const email = `staging-unlisted-disc-${Date.now()}@example.test`;
  const password = "StagingUnlistedDisc9!";
...

  {
    ...
    const { status, body } = await fetchJson("POST", url, {
      body: { email, password, displayName: "M5-T27 unlisted discover" },
    });
...

  {
    ...
    const r = await fetchJson("POST", u, {
      token,
      body: { notes: "M5-T27" },
      idempotencyKey: `m5t27-f-${Date.now()}`,
    });
  ...
  }
  {
    ...
    const r = await fetchJson("POST", u, {
      token,
      body: { mode: "full", respect_existing_manual_events: true, respect_existing_sources: true, notes: "M5-T27" },
      idempotencyKey: `m5t27-r-${Date.now()}`,
    });
  ...
  }
  {
    ...
    const r = await fetchJson("POST", u, {
      ...
      idempotencyKey: `m5t27-sel-${Date.now()}`,
    });
  ...
  }
  {
    ...
    const r = await fetchJson("POST", u, {
      ...
      idempotencyKey: `m5t27-asm-${Date.now()}`,
    });
  ...
  const warnFirst = valOutcome.overall_result === "warn";
  {
    ...
    const res = await fetch(url, {
      ...
      headers: {
        Authorization: `Bearer ${token}`,
        "Content-Type": "application/json",
        "Idempotency-Key": `m5t27-publish1-${Date.now()}`,
      },
      body: JSON.stringify(publishBody),
    });
    const rb = await readBody(res);
  ...
  if (!(await patchDraftVisibility(token, storyId, "unlisted", record, "15 — PATCH draft visibility_target unlisted"))) {
    printTable(steps);
    process.exit(1);
  }

  if (!(await republish(token, storyId, record, "16", "m5t27-pub-unlisted"))) {
    printTable(steps);
    process.exit(1);
  }

  {
    ...
    const r = await fetchJson("GET", url, {});
    const ok = r.status === 200 && r.body?.ok === true;
    record("17 — GET public by slug (unlisted still readable)", "GET", url, r.status, ok ? "passed on staging" : "attempted on staging but failed", ok ? {} : r.body);
    if (!ok) {
      printTable(steps);
      process.exit(1);
    }
...

  {
    ...
    record(
      "18 — GET discover (unlisted story must be absent)",
      "GET",
      discoverUrl,
      r.status,
  ...
  if (!(await patchDraftVisibility(token, storyId, "private", record, "19 — PATCH draft visibility_target private"))) {
    printTable(steps);
    process.exit(1);
  }

  if (!(await republish(token, storyId, record, "20", "m5t27-pub-private"))) {
    printTable(steps);
    process.exit(1);
  }

  {
    ...
    record("21 — GET public by slug (private — expect 404)", "GET", url, r.status, ok ? "passed on staging" : "attempted on staging but failed", { status: r.status });
    if (!ok) {
      printTable(steps);
      process.exit(1);
    }
...

  {
    ...
    const absent = r.status === 200 && r.body?.ok === true && !slugInDiscover(r.body, storySlug);
    record(
      "22 — GET discover (private story still absent)",
      "GET",
      discoverUrl,
      r.status,
      absent ? "passed on staging" : "attempted on staging but failed",
  ...
  record(
    ...
    "passed on staging",
    {
      ...
      contract:
        "Discover lists live public only. Unlisted and private are omitted from discover; unlisted remains slug-readable until moved to private.",
      next: "Restore public visibility via draft + checks + update-live if you want the story discoverable again.",
    },
  );
  ...
  console.log("\nM5-T27 staging verify: unlisted discovery/feed gating passed.");
}
